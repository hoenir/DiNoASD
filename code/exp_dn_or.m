...
%     %     crsDrawMatrixPalettised(square);
    ...
    for trial = 1 : current_block_size
        ...
        if current_side == 1 %COMPOUND LEFT, REFERENCE RIGHT
            crsSetDrawPage(12);
            crsClearPage(12,128);
            crsDrawMatrixPalettised(fix);
            ...
            crsDrawMatrixPalettised([param.ecc,0], IM_ref*255+1);
        else %COMPOUND RIGHT, REFERENCE LEFT
            crsSetDrawPage(12);
            crsClearPage(12,128);
            crsDrawMatrixPalettised(fix);
            crsDrawMatrixPalettised([-param.ecc,0],IM_ref*255+1);
        ...
        %start cycle (to interleave the two components of compound stimulus)
        ...
        if current_side_nonvert == 1 %reference stimulus is the one changing
            ...
            if current_side == 1 &  current_ref_or > 0%COMPOUND LEFT, REFERENCE RIGHT
                correct = (answer == 2);%correct if right is chosen
            elseif current_side== 1 & current_ref_or < 0%COMPOUND LEFT, REFERENCE RIGHT
                correct = (answer == 1);
            elseif current_side == 2 & current_ref_or > 0%COMPOUND RIGHT, REFERENCE LEFT
                correct = (answer == 1);
